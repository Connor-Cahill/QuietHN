...
func handler(numStories int, tpl *template.Template) http.HandlerFunc {
	...
	//	acts as ticker and every n seconds we update our cache
	go func() {
...
type storyCache struct {
	numStories int
	cache      []item
	expiration time.Time     //	expiration of cache
	duration   time.Duration //	duration of how long cache is good for
	mutex      sync.Mutex    //	mutex to stop race conditions amongst go routines
}

func (sc *storyCache) stories() ([]item, error) {
	sc.mutex.Lock()         //	Locks mutex so no other go routines come happenin
	defer sc.mutex.Unlock() //	Unlocks mutex when function is complete
